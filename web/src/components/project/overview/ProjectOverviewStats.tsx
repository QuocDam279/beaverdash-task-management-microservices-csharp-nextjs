...
export function ProjectOverviewStats({
  ...
  const renderProgressBar = (done: number, total: number, colorClass: string) => {
    if (total === 0) {
      return (
        <span className="text-[10px] text-slate-400 block mt-2.5 pt-0.5 border-t border-slate-100">
    ...
    return (
      <div className="space-y-1.5 mt-2.5 pt-2 border-t border-slate-100">
        <div className="flex justify-between items-center text-[10px] text-[#505258] font-medium">
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                ...
                <span className="text-3xl font-extrabold text-[#10b981] leading-none block">
                  {completedCount} <span className="text-xs font-semibold text-slate-400">chính</span>
                </span>
              </div>
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                ...
                <span className="text-3xl font-extrabold text-[#1868db] leading-none block">
                  {createdCount} <span className="text-xs font-semibold text-slate-400">chính</span>
                </span>
              </div>
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                ...
                <span className="text-3xl font-extrabold text-orange-600 leading-none block">
                  {upcomingDueCount} <span className="text-xs font-semibold text-slate-400">chính</span>
                </span>
              </div>
              <div className={`h-9 w-9 rounded-lg flex items-center justify-center group-hover:scale-105 transition-all shrink-0
                ${upcomingDueCount === 0 
