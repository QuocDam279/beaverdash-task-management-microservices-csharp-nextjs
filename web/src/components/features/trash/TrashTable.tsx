...
export function TrashTable({
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto border border-slate-200 rounded-lg shadow-2xs custom-chat-scrollbar" style={{ scrollbarGutter: "stable" }}>
      {tasks.length === 0 ? (
        <div className="py-12 text-center text-xs text-slate-400 italic bg-white select-none">
          Không tìm thấy công việc nào phù hợp với bộ lọc
        </div>
      ) : (
        <table className="w-full text-left border-collapse bg-white">
          <thead>
            <tr className="bg-slate-50/75 text-[10px] font-bold text-[#6b6e76] uppercase tracking-wider border-b border-slate-200 select-none">
              {/* Checkbox Header */}
              <th className="w-12 px-4 py-3.5 text-center">
                <input
                  ...
                  className="h-3.5 w-3.5 rounded border-slate-350 text-[#1868db] focus:ring-[#1868db] cursor-pointer"
                />
              </th>
              ...
              return (
                <tr
                  ...
                  className={`hover:bg-slate-50/50 transition-colors group ${
                    isChecked ? "bg-blue-50/20 hover:bg-blue-50/30" : ""
                  }`}
                >
                  {/* Checkbox Row */}
                  ...
                  {/* Thời gian xóa */}
                  <td className="px-6 py-4 text-slate-400 font-medium select-none">
                    {formattedTime}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2.5">
                      ...
                      <button
                        ...
                        className="text-slate-500 hover:text-[#1868db] hover:bg-slate-100/80 px-2 py-1 rounded transition-colors cursor-pointer text-xs font-bold flex items-center gap-1 border border-transparent hover:border-slate-200"
                      ...
                      {task.canPermanentDelete && (
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 focus-visible:opacity-100 transition-all duration-150 text-slate-400 hover:text-red-600 hover:bg-red-50 p-1.5 rounded border border-transparent hover:border-red-200 cursor-pointer"
